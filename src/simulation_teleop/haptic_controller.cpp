...
int main(int argc, char* argv[]) {
   ...
   // cout << redis_client_remote.getEigenMatrixJSON(ROBOT_DEFAULT_POS_KEY) << endl;
	...
	// auto R_starting = AngleAxisd(0 * M_PI/180.0, Vector3d::UnitZ()).toRotationMatrix();
	// R_device_robot *= R_starting;
	...
	teleop_task->setFilterCutOffFreq(0.01, 0.01);
    ...
    redis_client_local.addDoubleToReadCallback(0, DEVICE_GRIPPER_VELOCITY_KEYS[0], teleop_task->_current_gripper_velocity_device);
	 redis_client_local.addEigenToReadCallback(0, ROBOT_SENSED_FORCE_KEY, sensed_force_moment);  // added for haptic force response 
	...
	while (runloop) {
		...
		if(state == INIT) {
			...
			// update rotation matrix from mobile base heading (if rotating the robot base)
			// R_device_robot = R_starting * AngleAxisd(base_heading, Vector3d::UnitZ()).toRotationMatrix();
			// teleop_task->setDeviceRobotRotation(R_device_robot);

			// uncomment if sending simulation forces to haptics 
			// sensed_force_moment.tail(3).setZero();
			// teleop_task->updateSensedForce(sensed_force_moment);
			...
			teleop_task->_commanded_force_device = desired_force - kv_haptic * delayed_sigma_force * teleop_task->_current_trans_velocity_device;  // force sensor is already provided
